// Opt-in selectivity telemetry, bucketed by depth, stage, and node type.
...
namespace islay {
  ...
  struct SearchStats {
    ...
    struct Cell {
      ...
      // Low probes run only after failed high probes.
    ...
    // Includes leaves for the ProbCut cost denominator.
    std::uint64_t total_nodes = 0;

    // Per-move LMR re-search profile.
    ...
    // Stack-local counters flushed once at node exit.
    struct Acc {
